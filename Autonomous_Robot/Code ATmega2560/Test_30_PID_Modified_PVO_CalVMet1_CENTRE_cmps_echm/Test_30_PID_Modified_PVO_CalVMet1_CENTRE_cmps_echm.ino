...
volatile double margePerror= 1;
volatile double margeNerror= -1;
...
void PIDvitesse() {
  if (!inRange(Verror,margeNVerror,margePVerror) && !inRange(error,margeNerror,margePerror)){
    sommeVerror+=(double) Verror / ((double)(dt/Te));
    dVerror=(double) ((vitesseact - prevVact) / ((double) (dt/Te)));
    prevVerror = Verror;
...
void PIDorientation() {
  ...
  if (!inRange(Oerror,margeNOerror,margePOerror) && !inRange(error,margeNerror,margePerror)){
      sommeOerror+=(double) Oerror / ((double)(dt/Te));
      prevOerror = Oerror;
      vitesseOcmd =(double) (kpO * Oerror) - (kdO * vitesseOact) + kiO * sommeOerror;
    }
  else
    vitesseOcmd=0;
...
void PIDposition(){
  ...
  if (!inRange(dt,mindtR,maxdt)){
    ...
    if (!inRange(dtR,mindtR,maxdtR)){
        ...
        vitesseRact=(double) ((double) ((Rdist-prevRdist) / distUnitR) / (dt/Te*KR))*1.0e3; // mm/s
    }
    //Calcul vitesseLact
    if (!inRange(dtL,mindtL,maxdtL)){
        ...
        vitesseLact=(double) ((double) ((Ldist-prevLdist) / distUnitL) / (dt/Te*KL))*1.0e3; // mm/s
    }
    ...
    //Trapèze
    if (inRange(error,margeNerror,margePerror)){
        vitesseRcmd=0;
        vitesseLcmd=0;
    }
//    else if (dist<distacc){
//        vitesseRcmd = 200*Te;  //mm/s
//        vitesseLcmd = 200*Te;  //mm/s
...
//    }
        ...
        vitesseRcmd = 250*Te;  //mm/s
        vitesseLcmd = 250*Te;  //mm/s
        stopR=false;
        stopL=false;
    }
    else if (dist>cmd-distdec){
        sommeerror+=(double) error / ((double)(dt/Te));
        preverror = error;
        PIDP = (double) (kp * error) - (kd * vitesseact) + ki * sommeerror;
        vitessecmd=PIDP;
        if (vitessecmd!=0)
            PIDvitesse();
