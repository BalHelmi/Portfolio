...
volatile double kpV = 149;        //153
volatile double kdV = 80;         //60
volatile double kiV = 0.001;      //0.001
//PID orientation Parameters

volatile double kpO = 17;
volatile double kdO =16;
volatile double kiO = 0.0000067;
...
void PIDvitesse() {
  if (!inRange(error, margeNerror, margePerror)) {
    ...
    dVerror = (double) ((vitesseact - prevVact) / ((double)((dtR + dtL) / 2)));
    prevVerror = Verror;
