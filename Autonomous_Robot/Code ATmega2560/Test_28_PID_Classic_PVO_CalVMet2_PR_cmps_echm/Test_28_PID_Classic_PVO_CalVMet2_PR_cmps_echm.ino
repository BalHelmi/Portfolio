...
volatile double KR= (double) ((1024*2) / (PI*(8/100)));
volatile double KL= (double) ((600*2) / (PI*(8/100)));a
...
//          vitesseRact= (Rdist-prevRdist) / distUnitR / dtR;
...
//        vitesseRcmd=((double) ((Rcmd/100)*60) / ((double)(2*PI*(6.7/2/100)))); // RPMR
        ...
        if (inRange(Rerror,-0.2,0.2))
            ...
            pwmR = (double) aR *  ((double)(micros()/1.0e6));  //pwmR = (double) 2 * ( 100 / Rdistacc) * Rdist;
            stopR=false;
            //pwmRmax=pwmR;
        }
...
//          vitesseLact= (Ldist-prevLdist) / distUnitL / dtL;
//          // CONVERT nb_imp_par_sec => motor_wheel_rotation/min (RPM)
