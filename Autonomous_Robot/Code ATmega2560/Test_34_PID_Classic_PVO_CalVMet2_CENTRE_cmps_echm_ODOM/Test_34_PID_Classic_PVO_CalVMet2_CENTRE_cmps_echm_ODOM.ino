...
void PIDposition() {
  ...
  if (!inRange(dt, mindtR, maxdt)) {
    ...
    if (!inRange(dtR, mindtR, maxdtR)) {
      ...
      vitesseRact = ((double) (((double) ((Rdist - prevRdist) / distUnitR)) / (dtR * KR / Te)) * 1.0e2); // cm/s
    }
    //Calcul vitesseLact
    if (!inRange(dtL, mindtL, maxdtL)) {
      ...
      vitesseLact = ((double) (((double) ((Ldist - prevLdist) / distUnitL)) / (dtL * KL / Te)) * 1.0e2); // cm/s
    }
    ...
    //        vitesseRcmd =((double) (2/Te));
    //        vitesseLcmd =((double) (2/Te));
    ...
    else if ((dist >= cmd - distdec) && (inRange(dtL, mindtL, maxdtL) || inRange(dtR, mindtR, maxdtR))) {b / ((double)((dtR + dtL) / 2));
